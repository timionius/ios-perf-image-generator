...
class ImageGenerator:
    ...
    def generate_all(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                "files_generated": sum(len(v) for v in results.values() if isinstance(v, list)),
    ...
    def _generate_webp_images(self) -> List[Dict[str, Any]]:
        ...
        for framework, color in self.FRAMEWORKS.items():
            ...
            # Create image
            img = Image.new('RGBA', (self.config.image_size, self.config.image_size), 
                          (255, 255, 255, 0))
            draw = ImageDraw.Draw(img)
            
            # Draw background
            draw.rectangle([0, 0, self.config.image_size, self.config.image_size],
            ...
            # Draw border
            draw.rectangle([2, 2, self.config.image_size-2, self.config.image_size-2],
                          outline=color, width=4)
            
            # Save as WebP
            filepath = os.path.join(self.config.output_dir, f"test_image_{framework}.webp")
    ...
    def _generate_svg_images(self) -> List[Dict[str, Any]]:
        ...
        for framework, color in self.FRAMEWORKS.items():
            ...
            svg_path = os.path.join(self.config.output_dir, f"test_image_vector_{framework}.svg")
            self.logo_drawer.create_svg(svg_path, self.config.image_size, color, framework)
    ...
    def _generate_pdf_images(self) -> List[Dict[str, Any]]:
        ...
        for framework in self.FRAMEWORKS.keys():
            ...
            svg_path = os.path.join(self.config.output_dir, f"test_image_vector_{framework}.svg")
            pdf_path = os.path.join(self.config.output_dir, f"test_image_vector_{framework}.pdf")
    ...
    def _generate_png_fallback(self) -> List[Dict[str, Any]]:
        ...
        for framework, color in self.FRAMEWORKS.items():
            ...
            draw.rectangle([0, 0, scaled_size, scaled_size],
                          fill=(240, 248, 255, 255))
            
            # Simplified logo for PNG (using same drawer but scaled)
            self.logo_drawer.draw_logo(draw, scaled_size, color, framework, simplified=True)
            
            png_path = os.path.join(self.config.output_dir, f"test_image_vector_{framework}.png")
    ...
    def _generate_asset_catalogs(self) -> List[str]:
        ...
        for framework in self.FRAMEWORKS.keys():
            ...
            # Copy PDF if exists
            pdf_source = os.path.join(self.config.output_dir, f"test_image_vector_{framework}.pdf")
            ...
            contents = {
                "images": [
                    {
                        "idiom": "universal",
                        "filename": f"{framework}_image.pdf"
                    }
                ],
                "info": {
                    "author": "xcode",
