...
class LogoDrawer:
    ...
    def draw_logo(self, draw: ImageDraw, size: int, color: str, framework: str, simplified: bool = False):
    ...
    def _draw_react_logo(self, draw: ImageDraw, size: int, color: str, center: int, simplified: bool):
        ...
        for angle in [0, 60, 120]:
            ...
            ellipse_height = orbit_radius * 1.2 if not simplified else orbit_radius * 1.5
        ...
        # Draw center nucleus
        draw.ellipse([center - electron_radius, center - electron_radius,
                      center + electron_radius, center + electron_radius],
                     fill=color)
    
    def _draw_flutter_logo(self, draw: ImageDraw, size: int, color: str, center: int, simplified: bool):
        """Draw Flutter-style logo."""
        bar_width = size // 4
        bar_height = size // 12
        
        # Draw main vertical bar
        draw.rectangle([center - bar_width//3, center - size//3,
                        center + bar_width//3, center + size//3],
                       fill=color)
        
        # Draw horizontal bars
        draw.rectangle([center - bar_width//2, center - bar_height,
                        center + bar_width, center + bar_height],
                       fill=color)
        
        draw.rectangle([center - bar_width//2, center + bar_height,
                        center + bar_width//1.5, center + bar_height*3],
                       fill=color)
        
        if not simplified:
            # Add decorative dots
            dot_radius = size // 20
            draw.ellipse([center + bar_width//1.2, center + size//4,
    ...
    def create_svg(self, svg_path: str, size: int, color: str, framework: str):
        ...
        dwg.add(dwg.rect(insert=(0, 0), size=(size, size),
                        fill='#F0F8FF', stroke=color, stroke_width=4))
    ...
    def _create_swiftui_svg(self, dwg, size: int, color: str, center: int):
        ...
        dwg.add(dwg.circle(center=(center, center), r=radius,
                          fill='none', stroke=color, stroke_width=8))
        
        # S-curve
        path = dwg.path(d=f'M {center - radius//2} {center - radius//2} '
        ...
        # Text
        dwg.add(dwg.text('SwiftUI', insert=(center - 35, center + radius + 20),
                        fill=color, font_size=20, font_family='Helvetica'))
    ...
    def _create_react_svg(self, dwg, size: int, color: str, center: int):
        ...
        for angle in [0, 60, 120]:
            g = dwg.g()
            ellipse = dwg.ellipse(center=(center, center),
                                 rx=orbit_radius, ry=orbit_radius * 0.7,
    ...
    def _create_flutter_svg(self, dwg, size: int, color: str, center: int):
        ...
        # Vertical bar
        dwg.add(dwg.rect(insert=(center - bar_width//3, center - size//3),
                        size=(bar_width//1.5, size//1.5),
                        fill=color))
        
        # Horizontal bars
        dwg.add(dwg.rect(insert=(center - bar_width//2, center - bar_height),
